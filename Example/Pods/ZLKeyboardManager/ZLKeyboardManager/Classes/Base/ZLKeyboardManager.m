                ...
                while (superview.superview) {
                    if (![superview isEqual:moveContainerView]) {
                        ...
                        if ([superview isKindOfClass:UIScrollView.class]) {
                            ...
                            if (!scrollView.scrollEnabled || !scrollView.userInteractionEnabled || !currentResponder.keyboardCfg.preferScrollViewMove){
                                continue;
                            }
...
- (void)addInputToobarIfRequired {
    ...
    if (!textField.inputAccessoryView && textField.keyboardCfg.enableAutoToolbar) {
        ...
        if (@available(iOS 26.0, *)) {
            toolbar = toolbar ?: [[ZLToolBar alloc] initWithFrame:CGRectMake(0, 0, kScreenWidth, 58)];
        } else {
