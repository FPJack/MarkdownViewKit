...
@available(iOS 13.0, *)
public class GridTableAttachment: BaseAttachment<GridTableView> {
    /// 表格数据（二维单元格模型）。
    public var rows: [[GridCellModel]] {
        didSet {
//            view?.rows = rows
//            view?.reload()
            view?.updateData(data: rows)
        }
    }
    /// 表格配置。
...
//            customView.onRowStreamingFinished = completion
            customView.onStreamingFinished = completion
...
//            customView.startRowStreaming(rowInterval: rowInterval, animated: true)
            customView.startStreaming(data: rows, animation: true)
        } else {
            // 非动画：一次性显示完整表格。
